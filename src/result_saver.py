...
class ResultSaver:
    def __init__(
        ...
        self.valid_f1_criterion: Criterion = F1Criterion(
            valid_corpus, device,
            len(valid_corpus), num_sentences_per_batch
        )
    ...
    def save(
            self, name: str, i_so_far: int,
            binary_grammar: BinaryGrammar, unary_grammar: UnaryGrammar,
            commit: bool
    ):
        self.train_f1_criterion.refresh_dataloader()
        self.train_probability_criterion.refresh_dataloader()
        self.train_coverage_criterion.refresh_dataloader()

        train_f1: float = self.train_f1_criterion.score(
            binary_grammar, unary_grammar
        )
        train_prob: float = self.train_probability_criterion.score(
            binary_grammar, unary_grammar
        )
        train_cov: float = self.train_coverage_criterion.score(
            binary_grammar, unary_grammar
        )
        valid_f1: float = self.valid_f1_criterion.score(
            binary_grammar, unary_grammar
        )
        valid_prob: float = self.valid_probability_criterion.score(
            binary_grammar, unary_grammar
        )
        valid_cov: float = self.valid_coverage_criterion.score(
            binary_grammar, unary_grammar
        )
